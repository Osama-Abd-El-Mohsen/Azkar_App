...
####################### Helper Functions #########################
    ...
    def calculate_card_height(self, zekr, desc):
        base_height = 100  # Base height for card (adjust as needed)
    ...
    def get_azkar(self, zekr_cat: str):
    
        self.screen_manager.transition = WipeTransition()
        self.screen_manager.current = 'Loading Screen'
        cat_data = self.azkar_data[self.azkar_data.category == zekr_cat]

        cat_data.apply(self.get_cat_data, axis=1)
        # empty card and add to it adaptive_hieght
        self.screen_manager.get_screen('Azkar Screen_1').ids['list'].clear_widgets()
        for zekr, desc, count, ref in zip(self.zekr_list, self.description_list, self.count_list, self.reference_list):
            if len(desc) != 1:
...
####################### Build App Function #######################
    ...
    def on_start(self):
        ...
        if platform == "android":
            ...
            permissions = [Permission.POST_NOTIFICATIONS, Permission.READ_EXTERNAL_STORAGE,
